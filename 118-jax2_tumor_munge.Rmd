...
Reproducibility is when someone given your data can conduct the same analysis, arriving at the same parameter estimates and conclusions. The data processing steps of an analysis are a critical determinant of reproducibility. Ideally, this is performed in a breadcrumbs process, where each step is transparent. That's what R scripts do.

Here's an example of what I mean by that. I thought it would be interesting to try and pull this off using mostly tidyverse functions, if possible.
