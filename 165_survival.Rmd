...
The duration to an event is something that is calculated during data processing, or a data frame with start and end times can be passed directly into `survival` package functions, which will calculate survival time. Rather than show that, instead, I'm using this as an opportunity to illustrate how to deal with date/time variables more generally.
...
It turns out that dates can be a tricky variable in any computer language due to shifts such as time zones, the number of days in a month, daylight savings, leap years and even leap seconds. Strictly, unlike for seconds (which always have a fixed duration) time variables in units of minutes, hours, days, weeks, months and years (and more) might be approximate unless we account for these shifts.

The `lubridate` package has several utilities to deal with this when using date/time data. Our main concern is converting date or date/time entries into numeric values on whatever scale useful for our survival analysis algorithms.
...
Compute this interval and then divide by the period to calculate numeric values for variables for either the days, weeks or months of survival. Note how the interval spans a leap year, so this calculation is fairly sophisticated in its accuracy.
...
dataSet <- tibble(start_date, end_date) %>% 
