...
Do you have enough degrees of freedom for the model? Models with more parameters need more degrees of freedom to have a better chance at fitting. Rather than regressing on an average of technical replicates, run your regression using all technical replicate values, which doubles your degrees of freedom. Otherwise, you may not have data that is fittable by a complex model. In that case, choose a simpler model instead. 

Simulation *a priori* is the best way to know how many data points are necessary to test a given model. A three parameter model can be fit by as few as 12 well-behaved $x,y$ pairs. A five parameter model might be solvable with twice that many, but could likely need more. It all depends on the model and the quality of the data. 

Finally, and most importantly, most fitting problems can be avoided by proper experimental design. Design experiments that have several data points that "bracket" the expected parameter values symmetrically. This is done by choosing appropriate values of the $X$ variable for which to collect responses. 
...
If most of the data are on a plateau phase, the regression might hone in on an artifact in that region and solve that. If the parameter value of interest is within a data point desert, it might be lost for good!
...
Once you realize you have a nonlinear process, you'll need to lean on your scientific judgement to select the most appropriate model. One way is to simply reach up to your equation shelf to select one, based upon your expertise (or willingness to become an expert!) with the biological system.
