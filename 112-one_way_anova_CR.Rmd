...
If you haven't yet read the ANOVA big picture, [this is worth reading](\@ref(#introanova))
...
In other words, post hoc involves testing several hypotheses simultaneously. Each test risks type1 error. Therefore, this should be done in a way that keeps the cumulative, family-wise type1 error rate (FWER) below the type 1 error threshold that was set for the experiment; usually 5%.  
...
**Adjusting p-values for subsets**
Here's a three step procedure. 

Step1: Run the `pairwise.t.test` function, setting the argument `p.adjust="none"`. The output is a matrix we'll name `allPairs`, providing all possible comparisons.

Step2: Select from the `allPairs` matrix only the p-values that correspond to the comparisons you'd like to make. Name that vector of unadjusted p-values, `selectPairs`. This takes a bit of cleverness. `allPairs[,1]

Step3: Pass the `selectPairs` vector into the `p.adjust` function.

The output of step 3 are the adjusted p-values for the selected group of comparisons.

For example, we only want to compare all of the diets to casein.

The p-value output from the `pairwise.t.test` function is a matrix. 
...
The comparisons we want are all in the first column. Use matrix indexing skillz to grab only them:
...
Now pass those unadjusted p-values into the `padjust` function to generate adjusted p-values:

```{r}
p.adjust(selectPairs, method="bonferroni")
```
