...
First ask whether the variables are discrete or continuous. Then ask if they measured, ordered or sorted? Then ask which variables are controlled by the researcher and which are observed? The answers will point in the proper analytic direction. 
...
Sometimes experiments (eg, anything with an -omics) generate multiple dependent variables. $Y_j$ is the $j^th$ dependent variable. Here $j$ represents the number of dependent variables and can have values from 1 to $k$.  
...
Univariate experiments will generate $y_i$ measurements from $1 \times n$ replicates, whereas multivariate experiments will generate $y_{ij}$ measurements from $k \times n$ replicates.
...
The parameters $\beta_0$ and $\beta_1$ are fixed population constants (thus the Greek symbols) that define the relationship between the two variables. Here they represent the y-intercept and slope, respectively, of a straight regression line. 
