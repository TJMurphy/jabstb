...
After all that cleaning, he have a nice little dataframe that we can use to create a summary table. First, let's look and see what variables we have and decide what variables we want to display in our summary table: 
...
```
There are numerous ways you can modify your table with the `{flextable}` package. I recommend perusing the [package website](https://davidgohel.github.io/flextable/index.html) and trying out the different built in functions. To spruce up the table, I will add a footer and bold some of the variables in the "Characteristics" column. 

```{r}
cherry2018table %>% 
  gtsummary::as_flex_table() %>% 
  theme_vanilla() %>% 
  add_footer_lines(values = "NA indicates data not available.") %>% # adds footer
  bold(i = c(1, 2, 5, 13, 17), j = 1, bold = TRUE, part = "body") # bolds specified rows and columns 
```

Another cool feature of `{flextable}` is the option to use conditional formatting. To demonstrate, I will highlight rows in which the p-value is less than .05. To do this I have to use the `bg()` function. Within that function, the `i=` argument can be used to specify rows that you want to highlight while the `j=` function can be used to specify columns that you wish to highlight. Below, I use the `i=` argument along with the formula `~ p.value <.05` to specify that I wish to highlight rows in which the p-value is less than .05.     

You might notice that `p.value` is not the same as "p-value" which we see at the top of our table. Since we converted our gtsummary table to a flextable, I had to figure out what the column names were for the table to use in the `i=` argument. To do this, I ran `cherry2018table$table_header` which told me that the column name for the p-value column was in fact `p.value`. 
...
cherry2018table_final <- cherry2018table %>%
  ...
  bg(i = ~ p.value < .05,
     bg = "powderblue",
     part = "body") # specifying which rows to color
...
There is a package called [`{kablextra}`](https://cran.r-project.org/web/packages/kableExtra/vignettes/awesome_table_in_html.html) which is built off of the `kable()` function that is also great for making tables in RMarkdown and knitting to HTML.

### [gt](https://gt.rstudio.com/)

`{gt}` is a fairly new package but it is quickly rising in popularity. The package contains many intuitive functions for customizing each part of the table and all of the example tables look super clean and readable. However, at this time, the package only supports output to HTML.

### [reactable](https://glin.github.io/reactable/)

`{reactable}` tables work well with R Markdown documents and Shiny applications and is optimized for presentation via HTML. Additionally, the package offers expandable rows and nested tables and has a general emphasis on interactivity. 
