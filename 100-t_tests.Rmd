...
library(broom)
```
The t-tests, otherwise known as "Student's t tests", are for experimental designs that test hypotheses comparing responses of

* one group to a nominal standard or 
* two groups to each other, or 
* paired measures within replicates. 

The t-tests are for dependent variables involving measured data and when two or fewer levels of discrete (factorial) independent variable are tested. See Chapter \@ref(data).

To recall, measured data are on continuous scales. For example, we might use a mass balance to measure the mass of individual mice, and then record how their weight change in response to some factor at two different levels. Here, mass would be an equal interval measurement (the difference between 16 and 17 g is the same as the difference between 30 and 31 g). Mass is also continuous, in so far as a theoretically infinite number of values can exist between two units on the scale, depending upon the sensitivity of the measurements.
...
_Don't confuse a measurement with a replicate. Paired designs have two measurements from one replicate. For example, a measurement after placebo and then a second measurement after drug._

If you are not working with random independent replicates but are collecting data, then you are collecting data and not testing hypotheses. Inferential statistics are for hypothesis testing. There is nothing wrong with collecting data. There is no need to pretend they arose from a formal hypothesis-driven process.

This thinking is why I describe these as strong assumptions.
...
##### Residual error and normality testing

We have arrived at the simplest application of the linear model. Wherein one level $x_1$, or more commonly two levels $x_1,x_2$ of the independent variable $X$ predicts the response of the dependent variable $Y$ as
...
The focus of t-testing is to estimate for the value of $\beta_1$ and ask whether it differs statistically from zero. When $\beta_1=0$, a line drawn between the pair of points $x_1,y_1$ and $x_2, y_2$ has a slope of zero. This indicates $X$ has no effect on $Y$ and thus $E(Y)$ equals the value of the intercept, $Y = \beta_0$. 

Think of the intercept as the background or basal level the dependent variable. The slope as the magnitude of the change in the dependent variable caused by one unit of $X$. 

We assume that both $\beta_0$ and $\beta_1$ have fixed values...but we don't know what those values are (they are in Greek letters). Our job as scientists is to estimate these fixed values. 
...
That's a perfect model and it generates perfect data. Every time you run the chunk you'll get exactly the same result. Perfectly.

Of course, data are never that perfect in real life. 

Real life data has noise, which in jargon is called residual error. Residual error is, simply, an accounting trick. It is a way to account for all the sources of variation in our data that our perfect model cannot explain. 

The unexplained variation could be biological, or it could be measurement, or it could be both. Who knows? If we knew what caused it we would explain it by putting it in our model as another variable, like $Z$. But we can't explain it other than to blame it on random.
...
The R code model is updated below to account for both residual error and Gauss' discovery. Notice how we're adding a term to the model. This term is the random normal $N(\mu=0,\sigma=1)$ and it represents the residual error term, $\epsilon$, from equation\@ref(eq:lm).

If you run that model, you get a different result every time. The only way we have to explain the variation is that it comes from a random normal process.
...
So the normality assumption for t-testing (and other parametric statistical testing) is really about the normality of the residual error in the sample. And the reason we have this assumption is that all parametric models have a random normal term in them to explain that which we cannot yet explain. That random normal term modifies the influence of the beta coefficients on the values of $Y$ generated by the model.

But here is the big point of all this. Pragmatically, assessing the distribution of $\epsilon$ from small sample sizes is difficult. Which makes it hard to validate the weaker two assumptions. Simulate that code chunk above three times and try to convince yourself from those values alone that you have enough information to declare they are normally distributed (you don't).

As a researcher, particularly a small sample researcher, there are two ways to approach these weaker assumptions.

The first is to go all hybrid cross between Gauss and Alfred E. Neuman and just assume normality and not worry about it. This tends to be a safe bet when working with measured variables, particularly when we've verified that we are working within their linear range. Besides, at the core of the t-test and other parametric statistics are the theoretical distributions of means. The distributions of means ARE normally distributed via the central limit theorem.

The second approach is to run normality tests and to act on those results. That action is typically a data transformation or tossing decision.

Normality tests such as the Shapiro-Wilk (`shapiro.test`), tests the null hypothesis that the values of a random variable are normally-distributed. Levene's test `library(car), leveneTest` is useful to test the homogeneity of variance assumption. 

If their nulls are rejected, something must be done. The data might be normalized through log or reciprocal transformation. Then run the t-tests on these transformed values. Or the test result triggers an outlier removal protocol. Alternately, the decision tree calls for reverting to nonparametric analysis with the Wilcoxon tests instead of t-tests.\@(nonparametrics) 
...
If not dealing with large sample sizes it is probably futile to try to make these assessments. If they are used in an impactful way, such as in a decision tree to toss outliers or switch analysis and testing, their use should definitely be declared in advance, prior to running the experiment. 

Otherwise, what most are really asking from a normality test is for permission to p-hack, perhaps hoping to rescue "statistical significance" from an otherwise moribund result. 
...
The performance of t-tests and their nonparametric counterparts are virtually identical when the normality and homoscedasticity assumptions are met. Whereas nonparametric statistics can be more powerful when these assumptions are unmet. 

Yet, t-tests are generally held to be robust (aka, perform well) to violations of the weaker assumptions.

YMMV. Here's an illustration for when t-tests are not robust. 

```{r  echo=FALSE, fig.cap="Comparison of Wilcoxon test power to t-tests when sampling from skewed (lognormal) and normal distributions."}
knitr::include_graphics("images/pwr_tvw_tests.jpeg")
```

What's right for you? It depends on your experimental design and the data you intend to collect, and what you believe to be a scientfically significant result. You can, and should, Monte Carlo simulate the kind of results you expect to see in that next experiment. If you expect deviant data, model deviant data in these simulations. Compare different ways for a statistical analysis of the simulated "outcome" data. 

Which type of analysis performs best in pre-experiment simulations? Use that to guide whether you're going to be orthodox about the weaker assumptions, or not. 
...
I'm showing 4 different ways to calculate the numerators and denominators of $t$, which correspond to three very different experimental designs (the unpaired design has two ways to calculate $t$). 

The decision to use one or the other t-test is entirely scientific. It is based upon how the experiment is conducted. If you understand your experimental design, you'll know which t-test is applicable.
...
This is for analyzing experiments comparing the mean response $\bar y$ evoked by a single level of a predictor variable to a hypothetical population mean, $\mu$. The group is comprised of $n$ independent measurements, whose mean is $\bar y$.  The one sample t test has $n-1$ degrees of freedom.
...
Everybody knows that normal blood glucose in mice is 100 mg/dl. Blood glucose is measured in a random sample of 12 mice in which the insulin receptor gene has been disrupted. A one-sample t-test compares the mean of theirr measured glucose values to the nominal value, $\mu$=100 mg/dl. The experiment has 12 indendepdent replicates and the t-test has 11 degrees of freedom. 
...
When conducting power analysis during the experimental design phase, particularly when planning unequal sample sizes or for unequal variances, it is useful to compare how Welch and Student's perform against each other. It is p-hacking to make these decisions after the data are collected. 

#### Example

Irrespective of choosing to analyze the data using Welch's or Student's t-test, the experimental design is the same. 

A mouse model of diabetes has been established. It is used to test whether novel blood glucose lowering drugs work. A sample of 28 mice are randomly assigned to treatment with either a placebo $n$=14 or with a drug$n$=14. The drug group Blood glucose measurements are taken from all mice. Every measurement is independent of all other measurements. There are two means: $\bar y_{placebo}$ and $\bar_y{drug}$. 

The numerator of the t-statistic is the difference between these two group means. The denominator will be either Student's or Welch's way of calculating the standard error $se$.
...
To put this another way, that the sample shows a difference is a mathematical fact. We don't need a statistical test to tell us that. The test helps us to infer whether the populations that were sampled differ.
...
There is a school of thought that all testing should be two-sided. One reason for this is the enhanced stringency. Another is that confidence intervals of the estimate are more readily interpretable because they have both upper and lower bounds. You'll soon see (or noticed when we were in the parametrics) that one-sided tests have only one bound, the other side being infinite. Those are hard to interpret. A third reason is that when testing one-sided and the effect goes extreme but in the direction opposite of predicted, the null cannot be rejected formally. No inference could be made, formally.
...
To keep the stringency of two sides, we can run the t-test at `conf.level=0.975`. Problem solved.
...
I don't have a solution for the third point. If you started an experiment testing "greater" but the data show a statistical difference that is "less", then you should admit you were wrong. Sorry. To do otherwise is to invoke the HARK bias, "hypothesizing after receiving knowledge". 
...
A confidence interval has duality. It is an inferential statistic that also allows for estimating the accuracy of a sample. 
...
The CI is used inferentially. For the hypothesis rejection decision, the CI provides the same information as the p-value.

For example, imagine the sample mean from above (100, 95% CI 37.89 to 162.10) comes from an experiment to test the hypothesis that the mean differs from zero ($H_0:\mu=0$). Since the CI does not include 0, we can reject the null. You would also find a p-value less than 0.05 would be associated with this outcome.
...
I like to think about the 95% confidence interval as providing information about the accuracy of the mean; the range of values for what the true mean _might_ be based upon this one sample. Of course, when sampling from unknown hypothetical populations it's never possible to know if something is accurate with perfect certainty. However, it is useful from time to time assert how confident we are in an estimate in terms of the values it might otherwise taken on.

Therefore, be less concerned about whether a CI is wide or narrow. That is not the right question to ask. Be more concerned about whether the CI includes values that don't impress you scientifically. That thinking provides you a check against blind obedience to a p-value.
...
## Parameters, small samples, and reliability

A great deal of biomedical research revolves around experiments based upon small samples. A lot of the experimental designs, even those including many variables, boils down to comparing a group to some other or to a constant. 

Because of this the t-tests are easily the most frequently used test in biostatistics, whether used appropriately on measured variables, or sometimes inappropriately on sorted or ordered variables. 

The t-tests are based upon the sample parameters $\bar y, n, sd, where\ sem=\frac{sd}{\sqrt n}$ that are worth understanding intuitively.

The figure below illustrates the outcome of repeatedly sampling the __known__ population $N(\mu=100, \sigma=25)$. The coefficient of variation for this simulation is $cv=\frac{\sigma}{\mu}=0.25$, which my sense says is pretty typical for bio-variable with a well behaved signal to noise ratio. But YMMV.
...
```{r fig.cap="Simulation of the relationships between the sample parameters used in t-tests. These can represent either group parameters or the parameters for the difference between groups. "}
...
We __know__ the mean and standard deviation are 100 and 25, respectively. We coded them to be exactly that.  Yet, at low sample sizes, $n$, their estimates from the samples can, on average, be pretty far off. And these vary quite a lot, too. By random chance, a sample mean and standard deviation can range anywhere from being perfect to being awful crappy estimates of the population values. 

And we'd never know which one we got.

The researcher is not at fault for this. This happens even with exquisite technique and perfectly calibrated pipettes and other instrumentation, and slavish attention to detail. 

Biological systems have variance. When we do an experimental something to those systems we tend to provoke even more variance.
...
The second thing big thing this illustrator tells me is that $sd$ and $sem$ are not telling me the same thing.  The sample $sd$ measures dispersion in the sample, but that is also an estimate of $\sigma$, the dispersion in the population in the same way that $\bar y$ estimates the population mean $\mu$. 

As sample size increases the $sem$ gets lower and lower, because $sem=\frac{sd}{\sqrt n}$. 

The $sem$ is actually the standard deviation of the theoretical population of sample means, a concept which is derived from the central limit theorem^(In fact, the `sampler` function simulates the central limit theorem. Each blue point is a mean. Imagine calculating the mean and standard deviation of the 50 blue means at each value of $n$. The standard deviation of the 50 means at each $n$ is the $sem$.)  
...
knitr::kable(bind_rows(tidy(one2), tidy(oneL), tidy(oneG)), caption="One-sample t-test output. Read text for interpretation.")         
```
...
knitr::kable(bind_rows(tidy(up2), tidy(upL), tidy(upG)), caption="Unpaired t-test output. See text for interpretation. See interpretation of one-sample tests for details omitted here common to all tests.")   
```
...
knitr::kable(bind_rows(tidy(l2), tidy(p2), tidy(pL), tidy(pG)), caption="Paired t-test output. See text for interpretation. See interpretation of one-sample tests for details omitted here common in all tests.")
```
...
knitr::kable(data.y, caption="Summary statistics for a two group sample.")
```
...
A straight forward ways to add error bars is by use of the `stat_summary` function. A few ways exist to do this:

```{r fig.cap="More journals want small sample t-test data as scatter plots. Use stat_summary geoms to add error bars and use SD not SEM. The argument mult=1 coupled with mean_sdl draws +/- 1 SD"}
...
See Chapter \@ref(ttestmc) for extensive detail on performing Monte Carlo-based power calculations. Below you'll also find a streamlined version of a Monte Carlo function, `t.pwr`.

The `pwr` package in R has the `pwr.t.test` function. Given the same effect size you should find that a properly executed Monte Carlo will give you roughly the same result as a `pwr.t.test` function.

### pwr.t.test

It is very simple to execute `pwr.t.test`. This function requires that several decisions be made in advance.

First, it takes arguments for acceptable type1 error and for intended power. The standard for these in basic biomedical research is 5% and 80%, respectively, but you can use any level you deem appropriate. If you want to run at 86.75309% power and 1% alpha, go for it.

Two other arguments are `type`, which is the experimental design, and `alternative`, which is the hypothesis. These selections are scientific decisions.
...
__I strongly recommend AGAINST defaulting to Cohen's values for "large", "medium" and "small" effects, which apply to common standards in psychological research. They are much too ambiguous to be useful.__

Imagine you anticipate measurements that will have a standard deviation of 25 units. You know this because of familiarity with the assay. You estimate this value on the basis of your own preliminary data or on published information. It takes scientific judgment to settle on the SD value. 

You also believe the average response to a negative control or basal level of the predictor variable will be around 50 units. Also, based upon your scientific understanding of the system, in your mind, a minimally scientifically valid treatment effect will have a 100 unit response, or 2-fold above the negative control. 
...
People seem to have difficulty with Cohen's delta. It's nothing more complicated than a simple signal-to-noise estimate, just like is the t-statistic. You need to be able to predict delta. 

When designing an experiment temper your hoped for effect size with a sense of what you consider to be a minimally scientifically relevant effect size. In some systems, 2-fold is huge, whereas in other systems (with a lot of variation) a 2-fold response might be deemed insignificant.
...
This is a lean custom Monte Carlo function for t-test power. This is designed for you to assess power and sample size relationships prior to running an experiment. I find it to be more intuitive to use. YMMV.

For more expansive versions with more explanation, see Chapter\@ref(ttestmc).

`t.pwr()` takes sample size `n` as an argument, and returns the power of a t-test, given initialization parameters including the means, standard deviations, a correlation coefficient if paired, and configuration of the t-test arguments. 

Feed some expected experimental results into the initializer and it will behave as the population being sampled. The function will simulate a long run of random samples. It will conduct a t-test on each sample. For each t-test a p-value is collected.

For example, if you set a confidence level of 95%, every p-value less than 0.05 will be scored as a hit. Power = hits/total number of tests.

To customize for a specific application, modify the initializer values and the t.test arguments. 

What means and standard deviations should you estimate? Either the values you predict will happen, OR values that you believe would be the minimal scientifically meaningful effect size.

```{r unpaired students t monte carlo}
t.pwr <- function(n){
...
#Intitializers. Place expected values for the means and standard deviations of two groups to be compared here.
  ...
  # the monte carlo just repeats the random sampling i=ssims times. For each i, it runs a t-test on that sampe, grabs the p-value and places it in a growing vector, p.values[i]. Then it moves up to the next value of i.
  ssims=1000
  ...
  repeat{
    x=rnorm(n, m1, sd1); 
    y=rnorm(n, m2, sd2);
    #optional for paired, transform y from above given r coefficient
    #y=r*x + y*sqrt(1-r^2)
...
For paired design, n represents the number of pairs. 

From the power graph, it looks like a sample size of 6 or 7 per group is necessary.

This is not exactly identical to the `pwr.t.test` prediction of sample size. 
