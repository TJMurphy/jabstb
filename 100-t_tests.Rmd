...
The t-tests are based upon the sample parameters $\bar y, n, sd, where\ sem=\frac{sd}{\sqrt n}$. These are worth understanding intuitively.
...
Since we coded the values we __know__ the mean and standard deviation are 100 and 25, respectively. Yet, at low sample sizes, $n$, their estimates can, on average, be pretty far off. And these vary quite a lot, too. By random chance, a sample mean and standard deviation can range anywhere from being perfect to being awful crappy estimates of the population values. 

And we'd never know which one we got in our one crappy small sample.

The researcher is not at fault for this. This happens even with exquisite technique and perfectly calibrated pipettes and other instrumentation, and slavish attention to detail. This is just how random operates.

Biological systems have variance. When we do something to those systems we tend to provoke even more variance.
...
```{r fig.cap="Journals want data as scatter plots. Stat_summary geoms add error bars. The argument mult=1 coupled with mean_sdl draws +/- 1 SD"}
...
Paired t test data should always be plotted with point-to-point lines to illustrate the paired relationship of the measurements. In the paired design, each replicate is a pair of observations. 
...
The grouping variable is also used for creating a second plot to illustrate the values of the differences between measures. 
...
Think of the plot in A as showing how each replicate performed. The plot in B illustrates the paired t-test. It shows the difference between each replicate pair, the mean of those differences, and the confidence interval. As long as the confidence interval doesn't include the value of zero, the plot illustrates a statistical difference from the null of zero.
...
There are two ways to make these calculations, custom Monte Carlo's or package functions.
...
The `pwr` package in R has the `pwr.t.test` function. 
...
This function requires that several decisions be made in advance.

First, it takes arguments for acceptable type1 error and for intended power. The standard for these in basic biomedical research is 5% and 80%, respectively, but you can use any level you deem appropriate. If you want to run at 86.75309% power and at 1% alpha, go for it.
...
A power calculation for a one-sided hypothesis is illustrated for an unpaired design for a Cohen's $\delta=2$ :
...
People seem to have difficulty with Cohen's delta. It's nothing more complicated than a simple signal-to-noise estimate, just like is the t-statistic. You simply need to be able to predict delta. 
...
It is crucial to give the standard deviation a lot of thought. For example, here is a power calculation for when all else is equal but the the standard deviation is twice what was estimated above.
...
This is a lean custom Monte Carlo function for t-test power. This is designed for you to assess power and sample size relationships prior to running an experiment. I find it to be much more intuitive to use. YMMV on that point. The real advantage is the ability to customize virtually any expected outcome and to see which test performs best under those unique conditions.

For more expansive versions with more explanation, see Chapter\@ref(ttestmc).

This works differently than `t.test.pwr`. The `t.pwr()` below takes sample size `n` as an argument, and returns the power of a t-test. Feed `t.pwr` different values of `n` until it finds the power you need.
...
#Intitializers. Place expected values for the means and standard deviations of two groups to be compared here.
  ...
  alpha=0.05 #type1 error tolerance
    ...
    # the monte carlo just repeats the random sampling i=ssims times. 
  #For each i, it runs a t-test on that sampe, grabs the p-value 
  #and places it in a growing vector, p.values[i]. Then it moves 
  #up to the next value of i.
  ssims=1000
  ...
  repeat{
    ...
    #optional for paired, we must account for correlation of measurements within pairs
...
From the power graph, it looks like a sample size of 5 or 6 brackets 80% power.

This result is not exactly identical to the `pwr.t.test` prediction of sample size. 
